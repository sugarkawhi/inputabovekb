...
//     * @param activity The activity.
     ...
     */
    public static boolean isSoftInputVisible(final Dialog dialog) {
        return isSoftInputVisible(dialog, 200);
    }
...
//     * @param activity             The activity.
    ...
    public static boolean isSoftInputVisible(final Dialog dialog,
        ...
        return getContentViewInvisibleHeight(dialog) >= minHeightOfSoftInput;
    }

    private static int getContentViewInvisibleHeight(final Dialog dialog) {
        final FrameLayout contentView = dialog.findViewById(android.R.id.content);
     ...
     *
     * @param dialog The activity.
     * @param listener The soft input changed listener.
     */
    public static void registerSoftInputChangedListener(final Dialog dialog,
                                                        final OnSoftInputChangedListener listener) {
        final int flags = dialog.getWindow().getAttributes().flags;
        if ((flags & WindowManager.LayoutParams.FLAG_LAYOUT_NO_LIMITS) != 0) {
            dialog.getWindow().clearFlags(WindowManager.LayoutParams.FLAG_LAYOUT_NO_LIMITS);
        }
        final FrameLayout contentView = dialog.findViewById(android.R.id.content);
        sContentViewInvisibleHeightPre = getContentViewInvisibleHeight(dialog);
        onSoftInputChangedListener = listener;
        onGlobalLayoutListener = new OnGlobalLayoutListener() {
            @Override
            public void onGlobalLayout() {
                if (onSoftInputChangedListener != null) {
                    int height = getContentViewInvisibleHeight(dialog);
     ...
     * @param activity The activity.
     */
    public static void fixAndroidBug5497(final Dialog activity) {
